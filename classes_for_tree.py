...
class Tree_index_age_check():
    '''
    a class to store the information
    about whether the tree is aged or branch-lengthed
    and tracks indices for nodes 
    '''
...
def process_tree_info(node, tree_info=Tree_index_age_check()):
    '''
    assign  node indices to all nodes 
...
def sample():
    '''
    produces a sample tree if a newick file is not available
...
class Window(pg.GraphicsWindow):
    def __init__(self, title, tree_file=None,*args, **kwds):
        
        pg.GraphicsWindow.__init__(self, *args, **kwds)
        self.resize(1000,750)
        self.is_label = False

        if tree_file: #if file name given
            self.tree = ivy.tree.read(tree_file)
            if not self.tree: # if the tree file is invalid
                self.tree = sample() #uses the sample tree instead
        else: #if file name not given
            self.tree = sample() #uses the sample tree instead
        ...
        self.labels, 
        self.node_types, 
        self.connections] = info_processing.setup_plot(tree=self.tree)
    ...
    def set_aged(self):
        ...
        self.labels, 
        self.node_types, 
        self.connections] = info_processing.setup_plot(tree=self.tree, aged=True)
        self.update_tree_plot()
    ...
    def set_branched(self):
        ...
        self.labels, 
        self.node_types, 
        self.connections] = info_processing.setup_plot(tree=self.tree, branched=True)
        self.update_tree_plot()
